  ...
  return (
    <div className="flex flex-col space-y-7">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Con to="/payments" title="Buy Crypto" des="Buy crypto with robust providers" path="/flags/ic-banking.svg" />
        <Con to="/swap"     title="Swap"       des="Crypto to crypto conversion"     path="/flags/ic-swap.svg" />
        <Con to="/stake"    title="Stake"      des="Grow your crypto portfolio"      path="/flags/ic-savings.svg" />
      </div>
      ...
      <div className="flex flex-row items-start justify-between">
        <div className="flex flex-col font-semibold">
          ...
          <span className="text-[26px] font-bold">
            {user?.main_coin?.symbole}{user?.balance}
          </span>
          <span className="text-xs text-gray-400">
            Available Balance {user?.main_coin?.symbole}{user?.balance}
          </span>
        </div>
        ...
        <div className="flex flex-col space-y-6">
          <Container bg="bg-gradient-to-br from-[#0c67e7] to-[#063da9]" p="p-[26px]">
            <div className="text-white text-lg space-y-4 lg:p-[14px] relative">
              ...
              <img
                className="w-[120px] absolute top-[-20px] right-[-20px]"
                src="/flags/illustration-receipt.webp"
              />
              <div className="bg-[#073b9d] rounded-md flex flex-row w-full p-1">
                <input
                  placeholder="Email"
                  className="w-full pl-2 focus:outline-none bg-transparent placeholder-white/80"
                />
...
function Con({ title, des, path, to = '/' }) {
  return (
    <Link
      to={to}
      className="bg-[#f4f6f8] hover:bg-white hover:shadow-md transition-all duration-400 cursor-pointer font-medium rounded-xl p-[24px] flex flex-row items-center space-x-6"
    >
      <Ico path={path} />
