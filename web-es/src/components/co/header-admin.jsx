...
export default function Header() {
  ...
  // זהה ללוגיקה הישנה
  const navs = menuListAdmin.map((nav) => ({
    name: nav.name,
  ...
  return (
    <div className="h-[64px] bg-white px-[24px] flex flex-row items-center justify-between border-b border-dotted">
                ...
                {navs.map((nav, n) => (
                  <SheetClose key={n} asChild>
                    <Link
                      to={nav.link}
                      className={`${
                        nav.link === pathname ? 'bg-blue-50 text-blue-600' : 'text-gray-500'
                      } hover:bg-blue-100 hover:text-blue-600 cursor-pointer transition-colors duration-200 ${
                        nav?.space ? nav.space : 'space-x-3'
                      } py-3 px-3 rounded-md flex flex-row items-center`}
                    >
                      {nav.icon}
          ...
          <SheetFooter>
            <div className="flex flex-col items-center space-y-3">
              <div className="w-[48px] h-[48px] bg-gray-400 rounded-full flex items-center justify-center">
                <span className="font-semibold text-white text-[19px]">
      ...
      <div className="bg-white flex flex-row items-center space-x-4">
        ...
        <Link
          to="/account"
          className="border-[2px] border-blue-200 flex items-center justify-center h-[40px] w-[40px] rounded-full"
        >
