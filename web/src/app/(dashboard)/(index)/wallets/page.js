...
export default function Wallets() {
  ...
  const copyFunc = () => {
    navigator.clipboard.writeText(currencies?.find(c => c.iso === selectedCoin)?.wallet?.public_key || '')
      .then(() => {
  ...
  const maxAmountFunc = () => {
    var max = user?.balances?.[selectedCoin]?.amount;
  ...
  return (
    <div className="flex flex-col space-y-7">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <YourAssets user={user} currencies={currencies} selectedCoin={selectedCoin} setSelectedCoin={setSelectedCoin} isHome={false} />
              ...
              <div className="flex flex-col space-y-8 p-10">
                <div className="flex justify-center items-center">
                  <div className="p-4 bg-white rounded shadow">
                    <QRCodeSVG value={currencies?.find(c => c.iso === selectedCoin)?.wallet?.public_key || 'a'} size={170} marginSize={4} level="Q" />
                    {/* 'L' | 'M' | 'Q' | 'H' */}
                  </div>
                </div>
                <select className="w-full h-[50px] px-3 border rounded-md" value={selectedCoin} onChange={(e) => setSelectedCoin(e.target.value)}>
                  {
                    currencies?.map((currency, c) => (
                      <option key={c} value={currency.iso}>{currency.name} Wallet</option>
                    ))
                  }
                </select>
                <div className="flex fle-row justify-center items-center space-x-3 font-medium text-gray-400">
                  ...
                  <div onClick={copyFunc} className="hover:bg-gray-100 active:bg-gray-200 cursor-pointer rounded-full p-2">
                    <Clipboard />
                  </div>
                </div>
              </div>
                ...
                <div className="flex flex-col space-y-1">
                  ...
                  <div className="flex items-center border rounded-md px-3 py-1 focus-within:ring-2 ">
                    ...
                    <div className="flex flex-row items-center font-bold space-x-3 text-[14px]">
                      ...
                      <span className="text-gray-800 hover:text-gray-950 cursor-pointer" onClick={maxAmountFunc}>Max</span>
                    </div>
                  </div>
                </div>
                <div className="flex justify-center">
                  <Button variant='blue' >
                    <span className="font-semibold text-[15px]">Send {currencies?.find(c => c.iso === selectedCoin)?.name}</span>
                  </Button>
                </div>
              </form>
            }
...
const ConfirmTransaction = ({ open, setOpen, onConfirm }) => {
