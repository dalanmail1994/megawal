...
import ContainerViewAll from "./container-view-all";
...
export default function TransactionsCard({ isPagination }) {
                ...
                <div className="flex justify-end p-4 text-[15px] font-semibold">        
                    {
                        isPagination ? (
                            <div className="flex flex-row items-center space-x-[1px]">
                                <LinkBut title={'Previous Page'} click={prevPage} active={meta?.current_page > 1} />
                                <LinkBut title={'Next Page'} click={nextPage} active={meta?.current_page < meta?.last_page} />
                            </div>
                        ) : (
...
const LinkBut = ({title, active, click}) => {
    return (
        <span onClick={active ? click : () => {}} className={`transition-all duration-400 px-2 rounded-sm ${active ? 'hover:bg-gray-100 cursor-pointer' : 'font-medium text-gray-500'}`}>{title}</span>
    )
}