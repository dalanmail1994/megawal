...
export default function ChatBox({userId}) {
  ...
  const getMessage = async (index) => {
    try {
      const res = await getRequest(`/tickets/${index}/messages`);
      console.log(res)
      setMessages(res)
    } catch (err) {
  ...
  const sendMessage = async (e) => {
    e.preventDefault();
    const obj = {ticket_id: chosedTicket?.id, content: content, is_admin: pathname == "/admin/tickets"};
    try {
      const res = await postRequest("/messages", obj);
    ...
    <Container p="p-0 flex flex-row flex-grow">
      ...
      <div className="flex flex-col items-strech flex-grow">
        <div className="flex flex-row space-x-3 p-4 border-b">
          ...
          {
            (pathname == "/admin/tickets" && !(messages == null && chosedTicket == undefined)) && (
              <div className="flex items-center justify-center">
                <div onClick={() => setIsInfo(prev=> !prev)} className="hover:bg-gray-100 transition-all duration-400 cursor-pointer rounded-full w-7 h-7 flex items-center justify-center">
                  <svg width="20px" viewBox="0 0 24 24"><path fill="#637381" d="M13 3H4a1 1 0 0 0-1 1v16a1 1 0 0 0 1 1h9zm2 18V3h5a1 1 0 0 1 1 1v16a1 1 0 0 1-1 1zM7 8.5l4 3.5l-4 3.5z"></path></svg>
                </div>
              </div>
            )
          }
        ...
        <div className="flex flex-row h-full">
          <div className="flex flex-col items-stretch flex-grow w-0">
            <div className="flex flex-col p-4 space-y-2 h-[550px] overflow-y-scroll">
              ...
              {
                (messages == null && chosedTicket == undefined) && (
            ...
            <form onSubmit={sendMessage} className="flex-grow flex flex-row items-center px-4 border-t py-2">
              <InputChat placeholder="Type a message" value={content} onChange={(e) => setContent(e.target.value)} disabled={messages == null && chosedTicket == undefined} />
              <Button type="submit" disabled={messages == null && chosedTicket == undefined}>Send</Button>
            </form>
          </div>
          {/* side C */}
          <div className={`${isInfo ? '' : 'hidden'} w-full max-w-[280px] border-l flex flex-col space-y-6 items-center justify-between pt-8 pb-4`}>
            ...
            <div className="w-full flex flex-col space-y-4 p-3">
              ...
              <div className="flex flex-col space-y-[2px]">
                ...
                <Input className="" defaultValue="Support"  />
              </div>
              <Button className="">Update Admin Name</Button>
              <div className="p-3 bg-[#cafdf5] rounded-md flex flex-row items-start space-x-2">
                <svg className="text-sm w-[50px]" focusable="false" aria-hidden="true" viewBox="0 0 24 24"><path fill="#00b8d9" fillRule="evenodd" d="M22 12c0 5.523-4.477 10-10 10S2 17.523 2 12S6.477 2 12 2s10 4.477 10 10m-10 5.75a.75.75 0 0 0 .75-.75v-6a.75.75 0 0 0-1.5 0v6c0 .414.336.75.75.75M12 7a1 1 0 1 1 0 2a1 1 0 0 1 0-2" clipRule="evenodd"></path></svg>
...
const Message = ({ i, message, pathname }) => {
...
const LineTicket = ({ ticket, setChosedTicket }) => {
  // console.log(ticket)

  return (
    <div className="flex flex-row cursor-pointer items-center px-4 py-3 hover:bg-gray-100 space-x-3" onClick={() => setChosedTicket(ticket)}>
      <Cir>
        <svg className="p-[6px]" fill="white" focusable="false" aria-hidden="true" viewBox="0 0 24 24" data-testid="PersonIcon"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z"></path></svg>
      </Cir>
